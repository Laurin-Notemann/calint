...
export async function GET(request: NextRequest) {
  try {
    ...
    if (getUserErr) {
      logError(logger, getUserErr, { context: 'Failed to check if user exists' });
      return NextResponse.redirect(new URL('/error', request.url));
    }
    ...
    if (pipedriveUser.length > 0) {
      ...
      if (getCalendlyAccErr) {
        logError(logger, getCalendlyAccErr, { context: 'Failed to check if Calendly account exists' });
        return NextResponse.redirect(new URL('/error', request.url));
      }
      ...
      if (calendlyAcc.length > 0) {
        ...
        if (err) {
          logError(logger, err, { context: 'Failed to add Calendly account to user' });
          return NextResponse.redirect(new URL('/error', request.url));
        }
      }
    ...
    if (webhookError) {
      logError(logger, webhookError.error, { context: 'createWebhookSubscription' });
      if (webhookError.error.message === "Please upgrade your Calendly account to Standard") {
        const errorUrl = new URL('/error', request.url);
        errorUrl.searchParams.set('error-msg', "Your Calendly account needs at least a Standard subscription in order do create a webhook");
